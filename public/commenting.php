...
// error_reporting(0);
		...
		include_once('../includes/admin-functions.inc.php');

		global $smarty, $config, $visitorIp, $visitorId;
		...
		if(!empty($comment)){
          ...
          $extension = strtolower(array_pop($ext));
					...
					if($images = createThumbnailsComment($_FILES['image']['tmp_name'], $extension, $_FILES['image']['name'])){
						$comment['photoHash'] = $images['medium']['hash'];
						$comment['photoExt'] = $images['medium']['ext'];
					} else {
			...
			if(empty($errors)){
				...
				if($comment['commentId'] = dbQuery('comments', DB_INSERT, array('values'=>$comment))){
					echo('ok');
				}
