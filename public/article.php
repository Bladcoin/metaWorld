<?php
	...
	if(!empty($SECTIONS[$sectionId]) && $article = dbQuery('articles', DB_ARRAY, array('where'=>"sectionId='$sectionId' AND fileName='$fileName'"))){
		...
		if(!$smarty->is_cached($templateName, md5($fileName).SITE_LANG_POSTFIX)){
			...
			if($article['connect']) {
				$article['connect'] = json_decode($article['connect']);
			}
		...
		if(preg_match_all("/[0-9a-zA-Z]+/", $errorUrl, $matches)){
			...
			$where[] = "MATCH(name) AGAINST ('".mysqli_real_escape_string($MUSQLILINK, $query)."')";
