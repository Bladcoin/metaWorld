<?php
	...
	if(!empty($action['edit'])){
		...
		if(empty($errors)){
			$noItems = true;
			$smarty->assign('statuses', array_slice(lang('news:statuses'), 1));
	...
	/***** CUT FOR DEMO START *****/
		...
		if(empty($errors)){
			...
			if(empty($article['articleId'])){
				...
				if($article['articleId'] = dbQuery('articles', DB_INSERT, array('values'=>$article))){
					...
					/* сохранение изображения к статье */
	...
	} elseif(!empty($action['deleteNewsImage']) && !empty($article['articleId'])){ // удаление картинки
		$where[] = "articleId='$article[articleId]'";
		...
		if($articleId = dbQuery('articles', DB_VALUE, array('where'=>$where, 'fields'=>'articleId'))){
			...
			echo "writeStatus('".lang('news:errors:5')."', 'aError');\r\n";
			echo "window.setTimeout(\"writeStatus('')\", 5000);\r\n";
			exit;
		}
	} elseif(!empty($action['deleteNews']) && !empty($article['articleId'])){ // удаление статьи
		$where[] = "articleId='$article[articleId]'";
		...
		if(dbQuery('articles', DB_DELETE, array('where'=>$where))){
			...
			echo "writeStatus('".lang('news:errors:13')."', 'aError');\r\n";
			echo "window.setTimeout(\"writeStatus('')\", 5000);\r\n";
		}
		exit;
	} elseif(!empty($action['approveNews']) && !empty($article['articleId'])){
		$where[] = "articleId='$article[articleId]'";
		...
		if(dbQuery('articles', DB_UPDATE, array('where'=>$where, 'values'=>"status='visible'"))){
			echo "writeStatus('".lang('news:messages:3')."');\r\n";
	...
	if(empty($noItems)){
		...
		//d($article['type_content']);
		
		$sections = getSectionContent($MUSQLILINK, $type_content);

		$sections[0] = lang('news:allSections');
		foreach($articles as $articleId=>$article){
			$articles[$articleId]['statusName'] = lang('news:statuses:'.$articles[$articleId]['status']);
