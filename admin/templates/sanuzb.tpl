{include file="header.tpl" activeItem="sanuzb" title="Санатории и пансионаты Узбекистана"} 
...
{if $action.edit}
	<div class="content-frame" style="margin-bottom:10px;">
        <div class="content-frame-top"> 
            <div class="col-md-3 col-xs-3 nopadding_left"><h2>Санатории и пансионаты Узбекистана</h2></div>                                          
	...
	<form action="sanuzb.php" method="post" id="article" enctype="multipart/form-data">
        ...
        <div class="col-md-12 main main_buttons text-center">
		    <input class="btn btn-primary" type="submit" name="action[save]" value="&nbsp; {general:save} &nbsp;" onclick="document.getElementById('article').target='_self'; document.getElementById('article').action='{$GLOBAL_URL}/admin{if $SITE_LANG != ''}/{$SITE_LANG}{/if}/sanuzb.php?page={$page}'" /> &nbsp; 		
            <a class="btn btn-primary" href="sanuzb.php?page={$page}">{general:cancel}</a>
        </div>
    </form>
...
{elseif !$errors.access_denied}
    ...
    <div class="content-frame" style="margin-bottom:10px;">
        <div class="content-frame-top">                        
            <div class="col-md-6 col-xs-6 nopadding_left"><h2>Санатории и пансионаты Узбекистана</h2></div> 
            <div class="col-md-6 col-xs-6 nopadding_right">                                                
                <div class="pull-right"> 
                    <a class="btn btn-danger" href="sanuzb.php?action[edit]=true" data-toggle="tooltip" data-placement="left" data-original-title="{articles:addArticle}"><i class="fa fa-plus"></i></a>
      				<span>
                        {if $query || $status || $section}
                        <a class="btn btn-danger" href="sanuzb.php" ><span class="fa fa-list-alt"></span> {articles:showAllArticles}</a>
                        {else}
                ...
                <div class="panel panel-colorful">
                    <div class="panel-body">
                        ...
                        <div class="search_block_art">
                            <div class="col-md-5 col-xs-12 nopadding_left">{html_options options=$sections|smarty:nodefaults selected=$section name=section class="form-control select"}</div>
                            <div class="col-md-5 col-xs-12 nopadding_left">{html_options options=$statuses|smarty:nodefaults selected=$status name=status class="form-control select"}</div>
                    ...
                    <table class="table table-striped table-actions table-hover" id="articles">
                        ...
                        {foreach item=article from=$articles name=articles}
                            <tr id="article-{$article.articleId}">
                                <td style="line-height: 58px;" class="text-center" nowrap="nowrap">{$article.publishedOn|smarty:nodefaults}</td>
                                ...
                                <td class="btn-link-action text-center">
                                    <a class="btn btn-rounded" href="sanuzb.php?action[edit]=true&article[articleId]={$article.articleId}&page={$page}"><span class="fa fa-pen"></span></a>  
                                    <a class="btn btn-danger btn-rounded" onclick="deleteArticle({$article.articleId}, '{$article.title|smarty:nodefaults}');"><span class="fa fa-trash"></span></a>
                                </td>
                            </tr>
  						{/foreach}
                        </tbody>
                            ...
                            <tr>
                                ...
                                <td align="right" colspan="3">
                                    ...
                                    <ul class="pagination pagination-sm pull-right push-down-10 push-up-10">
                                        ...
                                        {if $number eq $page}
                                        <li class="active"><a href="sanuzb.php?page={$number}{if $query}&query={$query}{/if}{if $section}&section={$section}{/if}{if $status}&status={$status}{/if}">{$number}</a></li>
                                        {elseif $number eq '...'}
                                        ...
                                        {else}
                                        <li><a href="sanuzb.php?page={$number}{if $query}&query={$query}{/if}{if $section}&section={$section}{/if}{if $status}&status={$status}{/if}">{$number}</a></li>
                                        {/if}
                                        {/foreach}
                                    </ul>  
                                    {/if}
...
<script>
    ...
    $('.save_edit_image').click(function(e){	
        ...
        $.post('sanuzb.php', {id: id, desc: desc, link: link, action: 'editimage'}, function(data){
            if (data.update == 1){            
