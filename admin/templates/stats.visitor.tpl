	                    ...
	                    <tbody>
							...
							<tr class="{cycle values='odd,even'}">
								...
								{elseif $visitor.firstVisit.typeId eq 4}
									<a href="{$SITE_URL}/search.php?query={$visitor.firstVisit.searchQuery|escape:'url' nofilter}&page={$visitor.firstVisit.searchPage}" target="_blank">{$visitor.firstVisit.searchQuery|truncate:50}</a>
	            ...
	            <div class="table-responsive">
	                <table class="table table-bordered table-striped table-actions">
						...
						{foreach item=visit from=$visits name=visits}
	                   	...
	                   	<tr id="visit-{$visit.visitId}" {if $visit.isHidden}style="display:none"{/if}>
	                  		...
	                  		<td class="data" width="100%">
								...
								{elseif $visit.typeId eq 4}
									<a href="{$SITE_URL}/search.php?query={$visit.searchQuery|escape:'url' nofilter}&page={$visit.searchPage}" target="_blank">{$visit.searchQuery|truncate:60}</a>
								{elseif $visit.typeId eq 6}
									<a href="{$visit.requestUri}" target="_blank">{$visit.requestUri|truncate:60}</a>
									{if $visit.refererUrl}(<a href="{$visit.refererUrl}" target="_blank">{"{stats:referer}"}</a>){/if}
