        ...
        <div class="panel-body panel-body-table">
            <div class="table-responsive">
                <table class="table table-bordered table-striped table-actions">
                	...
                	<tr class="{$class}" onmouseover="this.oldClassName=this.className; this.className='{$class}Hov'" onmouseout="this.className=this.oldClassName">
                		<td class="data action" onclick="document.location='stats.visitors.php?domain={$referer.domain|escape:'url'|default:'unknown' nofilter}{if $filterTypeId}&filterTypeId={$filterTypeId}{/if}'" align="center">
	                		<a href="stats.visitors.php?domain={$referer.domain|escape:'url'|default:'unknown' nofilter}{if $filterTypeId}&filterTypeId={$filterTypeId}{/if}">
          		     ...
          		     {if $pageNums}             
                        <ul class="pagination pagination-sm pull-right push-down-10 push-up-10">
                            ...
                            {foreach from=$pageNums.pages item=number}
                                {if $number eq $page}
                                    <li class="active"><a href="stats.referers.php?page={$number}{if $filterTypeId}&filterTypeId={$filterTypeId}{/if}" class="pageNum">{$number}</a></li>
                                {else}
                				    <li><a href="stats.referers.php?page={$number}{if $filterTypeId}&filterTypeId={$filterTypeId}{/if}" class="pageNum">{$number}</a></li>
                                {/if}
                            {/foreach}
