{include file="header.tpl" activeItem="blocks" title="Блоки"} 
...
{if $action.edit}
	<div class="content-frame" style="margin-bottom:10px;">
        <div class="content-frame-top"> 
            <div class="col-md-3 col-xs-3 nopadding_left"><h2>Блоки</h2></div>                                          
	...
	<form action="blocks.php" method="post" id="article" enctype="multipart/form-data">
                                            ...
                                            <div class="file-previewzo-obzor" id="imageOptions">
                                                ...
                                                <div class="file-preview-thumbnails">
                                                    <div class="file-preview-frame" id="imageOptions">
                                                        <img src="{$article.images.original.url}" class="file-preview-image" alt="{$article.images.medium}">
                                                    </div>
        ...
        <div class="col-md-12 main main_buttons text-center">
		    <input class="btn btn-primary" type="submit" name="action[save]" value="&nbsp; {general:save} &nbsp;" onclick="document.getElementById('article').target='_self'; document.getElementById('article').action='{$GLOBAL_URL}/admin{if $SITE_LANG != ''}/{$SITE_LANG}{/if}/blocks.php?page={$page}'" /> &nbsp; 		
            <a class="btn btn-primary" href="blocks.php?page={$page}">{general:cancel}</a>
        </div>
    </form>
...
{elseif !$errors.access_denied}
    ...
    <div class="content-frame" style="margin-bottom:10px;">
        <div class="content-frame-top">                        
            <div class="col-md-6 col-xs-6 nopadding_left"><h2>Блоки</h2></div> 
            <div class="col-md-6 col-xs-6 nopadding_right">                                                
                <div class="pull-right"> 
                    <a class="btn btn-danger" href="blocks.php?action[edit]=true" data-toggle="tooltip" data-placement="left" data-original-title="{articles:addArticle}"><i class="fa fa-plus"></i></a>
      				<span>
                        {if $query || $status || $section}
                        <a class="btn btn-danger" href="blocks.php" ><span class="fa fa-list-alt"></span> {articles:showAllArticles}</a>
                        {else}
                        ...
                        {foreach item=article from=$articles name=articles}
                            <tr id="article-{$article.articleId}">
                                ...
                                <td class="btn-link-action text-center">
                                    <a class="btn btn-rounded" href="blocks.php?action[edit]=true&article[articleId]={$article.articleId}&page={$page}"><span class="fa fa-pen"></span></a>  
                            ...
                            <tr>
                                ...
                                <td align="right" colspan="3">
                                    ...
                                    <ul class="pagination pagination-sm pull-right push-down-10 push-up-10">
                                        ...
                                        {if $number eq $page}
                                        <li class="active"><a href="blocks.php?page={$number}{if $query}&query={$query}{/if}{if $section}&section={$section}{/if}{if $status}&status={$status}{/if}">{$number}</a></li>
                                        {elseif $number eq '...'}
                                        ...
                                        {else}
                                        <li><a href="blocks.php?page={$number}{if $query}&query={$query}{/if}{if $section}&section={$section}{/if}{if $status}&status={$status}{/if}">{$number}</a></li>
                                        {/if}
                                        {/foreach}
                                    </ul>  
                                    {/if}
...
<script>
    ...
    $('.save_edit_image').click(function(e){	
        ...
        $.post('blocks.php', {id: id, desc: desc, link: link, action: 'editimage'}, function(data){
            if (data.update == 1){            
