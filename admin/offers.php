<?php
	...
	if(!empty($action['edit'])){
		...
		$sections = getSectionTours($MUSQLILINK, $type_content);
		...
		if(empty($errors)){
			...
			$smarty->assign('sections', getSectionTours($MUSQLILINK, $type_content));
			$smarty->assign('statuses', array_slice(lang('articles:statuses'), 1));
	...
	} elseif(!empty($action['deleteArticle']) && !empty($article['articleId'])){ // удаление статьи
		$where[] = "articleId='$article[articleId]'";
