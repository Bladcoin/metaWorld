...
tinymce.init({
  ...
  language:"ru",
  plugins: [
    "bootstrap advlist autolink link image lists charmap print preview hr anchor pagebreak",
  ...
  toolbar: "bold italic underline | alignleft aligncenter alignright alignjustify | bullist numlist | formatselect | link unlink | imageupload media | forecolor backcolor  | preview code | removeformat | bootstrap ",
  relative_urls: false,
...
$(document).ready(function(){  
  $('#visible_maps').click(function(){	
    var ckVal = $(this).attr("data-val");
    $.ajax({
      url: 'functions.php',
      type: 'POST',
      data: {checkMaps: ckVal},
      dataType: 'json',
      cache: false,
      success: function(data){
        //console.log(data.code);
        $("#maps_down").val(data.code);
        
        if (data.code == 1){
         $('.upload-image').removeClass('alert-danger').addClass('alert-success').html(data.msg).fadeIn(1000).fadeOut(3000);
         $('label#visible_maps').attr('data-val', 0);	
        } 
        if (data.code == 0){
         $('.upload-image').removeClass('alert-danger').addClass('alert-success').html(data.msg).fadeIn(1000).fadeOut(3000);	
         $('label#visible_maps').attr('data-val', 1);
        } 
        
        /*
if (data == 1){
          $('#ajaxStatus').html('Карта включена').fadeIn(2200);
        	$('label#visible_maps').attr('data-val', 0);
        	$('#ajaxStatus').fadeOut(2000);
  
        }else{
          $('#ajaxStatus').html('Карта выключена').fadeIn(2200);
          $('label#visible_maps').attr('data-val', 1);
          $('#ajaxStatus').fadeOut(2000);
        }
*/
