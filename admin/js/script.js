...
/*
    if(DEMO_MODE){
		writeStatus(lang['demoMode'], 'error', 'statsAjaxStatus');
		return false;
	}
*/
...
function deleteUser(userId, userName){
	...
	postData("users.php?action[delete]=true&user[userId]="+userId);
	deleteUserMessage();
	return false;
}
...
function deleteAdminUserImage(userId){
	...
	postData("users.php?action[deleteAdminUserImage]=true&user[userId]="+userId);
	deleteImageMessage();
	return false;
}
...
function deleteProductStatus(product_status_id, name){
	if(!ajaxIsFree()) return false;
	if(window.confirm(lang['sureToDeleteOrderStatus'] + ' "'+name+'"?') == false) return false;
	writeStatus(lang['deletingOrderStatus'] + '...');
	postData("product-status.php?action[deleteProductStatus]=true&product_status[product_status_id]="+product_status_id);
	return false;
}
function deleteLabel(label_id, name){
	if(!ajaxIsFree()) return false;
	if(window.confirm(lang['sureToDeleteLabel'] + ' "'+name+'"?') == false) return false;
	writeStatus(lang['deletingLabel'] + '...');
	postData("labels.php?action[deleteLabel]=true&product_label[label_id]="+label_id);
	return false;
}
function deleteSocial(social_id, name){
	if(!ajaxIsFree()) return false;
	if(window.confirm(lang['sureToDeleteOrderStatus'] + ' "'+name+'"?') == false) return false;
	//writeStatus(lang['deletingOrderStatus'] + '...');
	postData("settings-social.php?action[deleteSocial]=true&social[social_id]="+social_id);
	deleteSocialMessage();
	return false;
}
