<?php 
    ...
    if(isset($_POST['checkMaps'])){
        $setting_map['codename'] = 'maps_down';
		$setting_map['value'] = $_POST['checkMaps']; 
		if( $_POST['checkMaps'] == 1){
  		    dbQuery('settings_maps', DB_REPLACE, array('values'=>$setting_map));
            $message = 'Карта включена!';
            echo json_encode(array('code'=>1, 'msg' => $message));
		}elseif ($_POST['checkMaps'] == 0){
  		    dbQuery('settings_maps', DB_REPLACE, array('values'=>$setting_map));
            $message = 'Карта выключена!';
            echo json_encode(array('code'=>0, 'msg' => $message));
	    }
    }
