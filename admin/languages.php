<?php
	...
	function removeLanguage($language){
		...
		foreach($tbl as $tableKey=>$void){
			if(!in_array($tableKey, array('languages', 'admin_history', 'banned_ips', 'admin_users', 'admin_user_images', 'subscribe', 'customers', 'customer_images', 'settings_social', 'settings_maps', 'settings_content'))){
				/* delete table */
	...
	function swapLanguages($language1, $language2){
		global $tbl, $tablePrefix, $DEFAULT_LANG, $MUSQLILINK;
		...
		$nLanguage['oldURL'] = mysqli_real_escape_string($MUSQLILINK, GLOBAL_URL.'/'.$nLanguage['codename']);
		$nLanguage['newURL'] = mysqli_real_escape_string($MUSQLILINK, GLOBAL_URL);
		$dLanguage['oldURL'] = mysqli_real_escape_string($MUSQLILINK, GLOBAL_URL);
		$dLanguage['newURL'] = mysqli_real_escape_string($MUSQLILINK, GLOBAL_URL.'/'.$dLanguage['codename']);
				...
				if($tableKey == 'settings'){
					if($langSettings = dbRawQuery("SELECT * FROM `$tableName` LIMIT 200", 'codename', 'value')){
						...
						foreach($langSettings as $codename=>$value){
							dbRawQuery("INSERT INTO `$newTableName` VALUES('".mysqli_real_escape_string($MUSQLILINK, $codename)."','".mysqli_real_escape_string($MUSQLILINK, $value)."')");
						}
					}
				}
			}
