...
if (!empty($action['clear_cache'])) {
	...
	if (!empty($settings['old_site_url'])) {
		...
		dbQuery('customers_images', DB_UPDATE, array('values' => "url = replace(url, '$settings[old_site_url]', '$settings[site_url]')"));
	...
	foreach ($settings as $codename => $value) if (!isset($config[$codename]) || $config[$codename] != $value) {
		...
		switch ($codename) {
			...
			case ('bot_id_strings'):
				...
				$setting['value'] = mysqli_real_escape_string($MUSQLILINK, $bots);
				dbQuery('settings', DB_REPLACE, array('values' => $setting));
				dbQuery('stats_user_agents', DB_UPDATE, array('values' => "isBot = IF(userAgent RLIKE '" . mysqli_real_escape_string($MUSQLILINK, $bots) . "', 1, 0)"));
			case ('bot_id_regexp'):
				continue 2;
			case ('query_fields_strings'):
				...
				$setting['value'] = mysqli_real_escape_string($MUSQLILINK, $queryFields);
				dbQuery('settings', DB_REPLACE, array('values' => $setting));
			case 'theme':
		...
		$setting['codename'] = mysqli_real_escape_string($MUSQLILINK, $codename);
		$setting['value'] = mysqli_real_escape_string($MUSQLILINK, $value);
		dbQuery('settings', DB_REPLACE, array('values' => $setting));
	}
