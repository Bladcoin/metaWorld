<?php
	...
	/***** CUT FOR DEMO START *****/
		...
		if(empty($section['toMain'])) $section['toMain'] = 0;
		...
		if(empty($section['fileName'])){
			...
			$section['subTemplateName'] = '';
			$section['artTemplateName'] = '';
			$section['status'] = 'hidden';
			if(dbQuery('sections', DB_VALUE, array('fields'=>'sectionId', 'where'=>"parentId='$section[parentId]' AND LOWER(fileName)=LOWER('$section[fileName]')"))) $errors['fileNameExists'] = true;
		} else {
			$section['parentId'] = 0;
			$section['type'] = 'plain';
			$section['templateName'] = '';
		...
		exit;
	} elseif(!empty($action['confirmDelete'])){ // подтвердите удаление
		/* загрузка раздел */
	...
	} elseif(!empty($action['deleteConfirmed'])){ // deleteConfirmed - подтвердить удаление
		/* удаление раздела */
		if($section = dbQuery('sections', DB_ARRAY, array('where'=>"sectionId='$section[sectionId]'"))){
			...
			if(isset($action['moveArticlesTo'])){
				if($action['moveArticlesTo'] == 'delete'){
					...
					dbQuery('articles', DB_UPDATE, array('where'=>"sectionId='$section[sectionId]'", 'values'=>"sectionId='$action[moveArticlesTo]', url=CONCAT('".mysqli_real_escape_string($MUSQLILINK, $SECTIONS[$action['moveArticlesTo']]['path'])."', '/', fileName, '.".mysqli_real_escape_string($MUSQLILINK, $config['file_extension'])."')"));
				}
			}
...
/*
elseif(!empty($action['sortAlphabetically'])){
		$sections = dbQuery('sections', DB_ARRAYS, array('fields'=>'sectionId, parentId, sortOrder', 'order'=>'parentId, name'));
		$sortOrder = 0;
		$prevParentId = 0;
		foreach($sections as $section){
			if($section['parentId'] != $prevParentId) $sortOrder = 0;
			$sortOrder++;
			...
			$prevParentId = $section['parentId'];
		}
		$messages['sorted'] = true;
	}
*/
	...
	if(empty($noItems)){
		...
		$sections = dbQuery('sections', DB_ARRAYS, array('fields'=>$fields, 'where'=>"footerMenu=1", 'start'=>($page-1)*$itemsPerPage - ($page > 1 ? 1 : 0), 'limit'=>$itemsPerPage + ($page > 1 ? 2 : 1), 'order'=>'sortOrder'));
