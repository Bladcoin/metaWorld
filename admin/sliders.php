<?php
	...
	/***** CUT FOR DEMO START *****/
		...
		if(empty($slider['fileName'])){
			if($maxId = dbQuery('sliders', DB_VALUE, array('fields'=>'MAX(sliderId)'))){
				$slider['fileName'] = $maxId + 1;
			} else {
				$slider['fileName'] = 1;
			}
		} elseif(empty($slider['sliderId'])){
			if(dbQuery('sliders', DB_VALUE, array('fields'=>'sliderId', 'where'=>"LOWER(fileName)=LOWER('$slider[fileName]')"))) $errors['fileNameExists'] = true;
		} else {
			if(dbQuery('sliders', DB_VALUE, array('fields'=>'sliderId', 'where'=>"sliderId<>'$slider[sliderId]' AND LOWER(fileName)=LOWER('$slider[fileName]')"))) $errors['fileNameExists'] = true;
		}
		if(preg_match('/^(page\d+|index)$/i', $slider['fileName'])) $errors['fileNameProhibited'] = true;
		if(preg_match('/[^0-9a-zA-Z\-_\.,]/i', $slider['fileName'])) $errors['fileNameCharacters'] = true;
			...
			if(empty($slider['sliderId'])){
				...
				if($slider['sliderId'] = dbQuery('sliders', DB_INSERT, array('values'=>$slider))){
					if(!empty($_FILES['image']) && is_uploaded_file($_FILES['image']['tmp_name'])) {
						...
						if($images = createThumbnailsSlider($_FILES['image']['tmp_name'], $extension, $slider['sliderId'], '', $_FILES['image']['name'])){
							$sliderImages = dbQuery('slider_images', DB_ARRAYS, array('where'=>"sliderId='$slider[sliderId]'"));
