...
function dbQuery($table, $type, $params = array())
{
	...
	switch ($type) {
		...
		case DB_VALUE:
			$result = dbRawQuery("SELECT $fields FROM $table $join $where $group $order $limit", $indexKey, $valueKey);
			if (is_array($result)) {
				list($void, $value) = current($result[0]);
				return $value;
			} else {
				return false;
			}
		...
		case DB_UPDATE:
			...
			return mysqli_affected_rows($MUSQLILINK);
			break;
...
function dbRawQuery($query, $indexKey = '', $valueKey = '')
{
	...
	global $dbTotalQueries, $dbQueryTime, $dbHost, $dbUser, $dbPass, $dbName, $MUSQLILINK;
	...
	do {
		$attempt++;
		try {
			$result = mysqli_query($MUSQLILINK, $query);
		} catch (Exception $error) {
			return $error;
		}
		if (!$result && mysqli_connect_errno()) {
			mysqli_free_result($MUSQLILINK);
