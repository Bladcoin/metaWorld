<?php
    ...
    function buildLanguagesArray(){
    	global $LANGUAGES;
    	$LANGUAGES = dbQuery('languages', DB_ARRAYS, array('fields'=>'languageName, codename, url, isDefault, status', 'indexKey'=>'codename', 'order'=>'sortOrder'));
    	$langExport = "<?php\r\n";
     ...
     */
    function replaceLanguageTags($source, Smarty_Internal_Template $template){
    	global $adminLang, $L;
    ...
    */
    ...
    function createThumbnailsComment($fileName, $extension, $origFileName = ''){
        global $config, $smarty;
        ...
        // $rawDir = tmpDirName(SITE_ROOT."/uploads/comments");
        $rawDir = md5(time()+rand(0,555555)+'AbraCadabra');
        $destDir = SITE_ROOT."/uploads/comments/".$rawDir;
        if(!is_dir($destDir) && !@mkdir($destDir)) return false;
        $smarty->assign('imageDir', $destDir);
        /* saving thumbnails */
        foreach(array('medium') as $size){
            $images[$size] = array();
            $images[$size]['fileName'] = $imageName."photo.$extension";
            $images[$size]['hash'] = $rawDir;
            $images[$size]['ext'] = $extension;
            $thumb = createThumbnail($fileName, $config[$size.'_thumb_width'], $config[$size.'_thumb_height'], "$destDir/".$images[$size]['fileName'], $config['thumbnail_quality'] );
        ...
        // $images['original']['width']  = $image[0];
        // $images['original']['height'] = $image[1];
        // $images['original']['url'] = SITE_URL."/uploads/comments/$rawDir/".$images['original']['fileName'];
        // $images['original']['codename'] = 'original';
        @unlink($fileName);
        return $images;
    }
     ...
     * @return array
     */
    function getSectionOptions($skipId = 0, $maxDepth = 99, $prefix = '', $separator = ' -- ', $productType = ''){
     ...
     * @return array
     */
    function getSectionTours($skipId = 0, $type_content, $maxDepth = 99, $prefix = '', $separator = ' &#4347; '){
        global $config, $TREE, $SECTIONS, $adminUser;
    ...
    function buildSectionArrays($buildLang = array()){
    	global $SECTIONS, $TREE, $config, $MUSQLILINK;
    	...
    	foreach($SECTIONS as $sectionId=>$section){
    		...
    		if(!empty($section['parents'])){
    			...
    			/* build path and url */
    			/* count child levels */
                // $section['parents'] = array_reverse($section['parents']);

                // Этот тупой баг был наконец исправлен
                    foreach($section['parents'] as $parentId){
                        if($SECTIONS[$parentId]['subTemplateName']) $last = $SECTIONS[$parentId]['subTemplateName'];
                    }
                // 
    			foreach($section['parents'] as $parentId){
    				...
    				if(empty($section['templateName']) && !empty($SECTIONS[$parentId]['subTemplateName'])){
    					$section['templateName'] = $SECTIONS[$sectionId]['templateName'] = $last;
    				}
    		...
    		if($SECTIONS[$sectionId]['url'] != $tmpUrl){
    			...
    			dbQuery('sections', DB_UPDATE, array('where'=>"sectionId='$sectionId'", 'values'=>array('url'=>mysqli_real_escape_string($MUSQLILINK, $tmpUrl))));
    			dbQuery('articles', DB_UPDATE, array('where'=>"sectionId='$sectionId'", 'values'=>"url=CONCAT('".mysqli_real_escape_string($MUSQLILINK, $SECTIONS[$sectionId]['path'])."', '/', fileName, '.".mysqli_real_escape_string($MUSQLILINK, $config['file_extension'])."')"));
    		}
    	}
    ...
    function writeHtaccess($buildLang = array()){
        ...
        // if($fh = fopen($siteRoot.'/.htacess', 'w')){
