...
<footer class="footer">
    <div class="container footer-container">
            <div class="footer__top">
                <div class="footer__row">
                    <div class="footer__desc">
                     {$main.content_two|replace:"<p>":'<h2 class="footer__title title">'|replace:'</p>':'</h2>' nofilter}
                      <div class="footer__social">
                        <a href="{$THEME_URL}/#" class="footer__social-link">
                    ...
                    <div class="footer__info">
                      <div class="footer__tel"><a href="tel:{$config.phone|replace:'(':''|replace:')':''|replace:'-':''|replace:' ':''}">{$config.phone|replace:'(':''|replace:')':''|replace:'-':''}</a></div>
                      <h3 class="footer__subtitle">{$config.address}</h3>
                      <div class="footer__email"><a href="mailto:info@atllc.uz">{$config.feedback_email}</a></div>
                      <nav class="footer__nav">
                        <ul class="footer__list">
                          {foreach item=navItem1 from=$TREE name=navItems1}
                           {if $navItem1.status eq 'visible' && $navItem1.top_menu eq '1'}
                          
                            <li><a href="{$navItem1.url}">{$navItem1.name}</a></li>
            ...
            <div class="footer__bottom">
                <div class="footer__copy">{$config.company_name} {$smarty.now|date_format:'%Y'}</div>
            </div>
    </div>
</footer>
  ...
  ymaps.ready(init);
    function init() {
      ...
      function createMenuGroup (group,mainlat) {
        // Пункт меню.
        var menuItem = $('<li><a href="{$THEME_URL}/javascript:;" mainlat="'+mainlat+'">' + group.name + '</a></li>'),
